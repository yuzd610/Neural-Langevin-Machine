...
mnist_test_full = datasets.FashionMNIST(root='./data', train=False, download=True, transform=transform)
...
# Load matrix
loaded_compressed_matrix = np.load("compressed_matrix.npy")  # (N_VAE, N_image)

# Choose an index to visualize
sample_idx = 0

# Take the corresponding sample for decoding
z_sample = torch.tensor(loaded_compressed_matrix[:, sample_idx]).float().to(device).unsqueeze(0)  # (1, N_VAE)
...
# Get the original image (before any [-1, 1] scaling)
orig_img = images_full[sample_idx].squeeze().cpu()

# Plotting Original and Reconstructed side by side
...
axes[1].set_title(f"Reconstructed ({N_VAE}-dim Latent)")
axes[1].axis('off')
