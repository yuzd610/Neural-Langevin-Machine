...
# Dynamically get image count to prevent mismatch between N_image and actual filtered count
matrix = transformed_matrix.reshape(transformed_matrix.shape[0], -1).T  # Shape: (N, Actual_N_image)
...
for epoch in tqdm(range(n)):
    for batch in dataloader:
        with torch.no_grad():
            ...
            # Model Phase: PCD (Persistent Contrastive Divergence) Langevin Dynamics
            for j in range(k):
            ...
            X[:, :, 0] = X[:, :, -1]  # Update the persistent chain
            x_model = X[:, :, -1]
